...
,mn[c(37:44),] ) %>% select(mn,Point.Forecast,price,Hi.80,Lo.80,Lo.95,Hi.95) %>% mutate(Hi=(Hi.80+Hi.95)/2,Low=(Lo.80+Lo.95)/2) %>% select(mn,price,Point.Forecast,Hi,Low)


detect_last_gt<-
detect_last %>% select(mn,price,Hi)%>% gt() %>% 
...
select(mn,price,Point.Forecast,Hi,Low)%>%  
pivot_longer(!mn,names_to = "cat",values_to = "price") %>% 
filter(cat %in% c("price","Hi"))%>%  
ggplot(aes(x=mn,y = price,color=cat,group=cat))+geom_line(size=0.9)+
scale_x_datetime(date_breaks ="month" ,date_labels = "%b-%Y")+theme_bw()+
labs(x="",y="")+
scale_y_continuous(breaks = c(25000,35000,45000,55000,65000,75000,85000,95000,105000))+
...
[S&P500 Model:](https://afaaqitc.github.io/SP500/#Last_Year_Table){target="_blank"}

[Oil-CL=F Model:](https://afaaqitc.github.io/OIL-Forecast/#Detect_Forecast_Model_Fore_Last_8_Months){target="_blank"}
